...
export default function ThreadView({ threadId }: { threadId?: string }) {
  ...
  const messages = (data?.thread?.messages ?? []) as any[];
  ...
  useThreadRealtime(threadId, {
    onMessage: ({ message }: { message: any }) => {
      ...
      qc.setQueryData(["thread", threadId], (old: any) => {
        ...
        const list = old.thread.messages ?? [];
        if (list.some((m: any) => m.id === message.id)) return old;
        const next = [...list, message].sort(
          (a: any, b: any) =>
            new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime(),
        );
        return { ...old, thread: { ...old.thread, messages: next } };
      });
    },
  });
  if (!threadId)
