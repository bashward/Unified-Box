      ...
      {loading ? (
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          <div className="rounded-2xl border border-border bg-card p-4">
            <div className="text-xs text-muted-foreground mb-1">Messages (7d)</div>
            ...
            <div className="mt-3 grid grid-cols-2 gap-2 text-sm">
              ...
              <div className="rounded-xl border border-border/60 bg-muted/50 p-3">
                ...
                <div className="font-medium">{data?.byChannel.whatsapp ?? 0}</div>
              </div>
            </div>
          </div>

          <div className="rounded-2xl border border-border bg-card p-4">
            <div className="text-xs text-muted-foreground mb-1">Avg First Response (24h)</div>
            <div className="text-2xl font-semibold">
              {data?.avg?.avgMinutes != null ? `${data.avg.avgMinutes} min` : "—"}
            </div>
            <div className="text-xs text-muted-foreground mt-1">
              {data?.avg?.avgMs != null ? `${data.avg.avgMs} ms` : "No pairs found"}
            </div>
          </div>

          {/* Placeholder for future chart or tile */}
          <div className="rounded-2xl border border-border bg-card p-4">
            <div className="text-xs text-muted-foreground mb-1">Notes (24h)</div>
